...
def create_11_pointcloud_dir(project_dir, lidar_name):
    ...
    if os.path.exists(POINTCLOUD_dir) == False:
        os.mkdir(POINTCLOUD_dir)

    outputDir = project_dir + "/11_POINTCLOUD/"+lidar_name
...
def get_lidar_type(project_dir, lidar_name):
    # 1#CAR and LiDAR_1
    if str(project_dir).find("DSX116") != -1 and str(lidar_name).find("1") != -1:
        return str(128)
    # 1#CAR and not LiDAR_1
    elif str(project_dir).find("DSX116") != -1 and str(lidar_name).find("1") == -1:
        return 32
